...
// Use
//
// gcc -o tutorial03 tutorial03.c -lavformat -lavcodec -lswscale -lz -lm `sdl2-config --cflags --libs`
...
//Data type to store the audio data while waiting for SDL to process the previous
...
int main(int argc, char *argv[]) {
    ...
    //------- aXXXX: for audioStream -------//
    //------- pXXXX: for videoStream -------//
    ...
    printf("Audio codec sample_rate: %d\n", wanted_spec.freq);
    wanted_spec.format = AUDIO_S16SYS;//signed 16-bit sys==endian by the system
    wanted_spec.channels = aCodecCtx->channels;//Number of audio channels
    wanted_spec.silence = 0;
    wanted_spec.samples = SDL_AUDIO_BUFFER_SIZE;//Size of buffer that SDL provided, between 512 to 8196
    wanted_spec.callback = audio_callback;//callback function comes
    wanted_spec.userdata = aCodecCtx;//data to give to the callback fucntion
    ...
    SDL_PauseAudio(0);
    //a queue is used to stored the audio data while SDL is busy playing previous
    //data, and when it's free it will come back, take sth from the queue and ...
    ...
    sws_ctx = sws_getContext(pCodecCtx->width, pCodecCtx->height,
        pCodecCtx->pix_fmt,
        pCodecCtx->width/scale_ratio, pCodecCtx->height/scale_ratio,
        AV_PIX_FMT_YUV420P,

...
// Change the SDL_Overlay to SDL_Texture & SDL_Screen
...
void video_display(VideoState *is) {
    ...
    if(vp->frame) {
        ...
        // rect.w = w;
        // rect.h = h;
        SDL_LockMutex(screen_mutex); // mutex locked
        ...
        SDL_UnlockMutex(screen_mutex); //unlock line 310
    }
}
...
void alloc_picture(void *userdata) {
    ...
    // }
    //@new: alloc the AVFrame* here if it doesn't match the required
    //the criteria bigger/smaller is checked from queue_picture function
    ...
    av_image_fill_arrays(vp->frame->data, vp->frame->linesize,
        frameBuffer,
        AV_PIX_FMT_YUV420P, is->video_ctx->width, is->video_ctx->height, 1);//by default, align = 1;
...
int queue_picture(VideoState *is, AVFrame *pFrame) {
    ...
    if(vp->frame) {
        ...
        // Convert the image from pFrame into YUV format that SDL uses
        // From pFrame to vp->frame
...
int video_thread(void *arg) {
    ...
    // Size of new frames
    int scale_ratio = 1;
    int scaleWidth = is->video_ctx->width/scale_ratio;
    int scaleHeight = is->video_ctx->height/scale_ratio;
    printf("Screen video %dx%d\n", scaleWidth, scaleHeight);
